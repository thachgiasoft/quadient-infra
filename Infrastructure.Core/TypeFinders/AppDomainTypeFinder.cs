...
namespace Infrastructure.Core.TypeFinders
{
    /// <summary>
    /// A class that finds types needed by core by looping assemblies in the 
    ...
    public class AppDomainTypeFinder : ITypeFinder
    {
        ...
        private string _assemblySkipLoadingPattern = "^System|^mscorlib|^Microsoft|^CppCodeProvider|^VJSharpCodeProvider|^WebDev|^Castle|^Iesi|^log4net|^NHibernate|^nunit|^TestDriven|^MbUnit|^Rhino|^QuickGraph|^TestFu|^Telerik|^ComponentArt|^MvcContrib|^AjaxControlToolkit|^Antlr3|^Remotion|^Recaptcha|^DevExpress|^WebGrease";
        ...
        /// <summary>Gets or sets wether core should iterate assemblies in the app domain when loading core types. Loading patterns are applied when loading these assemblies.</summary>
        ...
        /// <remarks>If you change this so that core assemblies arn't investigated (e.g. by not including something like "^core|..." you may break core functionality.</remarks>
        ...
        public IEnumerable<Type> FindClassesOfType(Type assignTypeFrom, IEnumerable<Assembly> assemblies, bool onlyConcreteClasses = true)
        {
            ...
            {
                var msg = string.Format("Assembly to Load : {0} --> Required -->", lastAssembly != null ? lastAssembly.FullName : string.Empty);
                foreach (var e in ex.LoaderExceptions)
                    msg += e.Message + Environment.NewLine;

                var fail = new Exception(msg, ex);
                Debug.WriteLine(fail.Message, fail);
                var errMsg = fail.Message;
                if (errMsg.Length >= 30000)//event log default max char
                    errMsg = errMsg.Substring(0, 30000);
        ...
        /// <returns>A list of assemblies that should be loaded by the core factory.</returns>
        ...
        /// <returns>True if the assembly should be loaded into core.</returns>
